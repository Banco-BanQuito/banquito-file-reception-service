...
public class CsvBatchParserImpl implements ICsvBatchParser {
    ...
    public ParsedBatch parse(InputStream inputStream, String serviceType, String clientRuc) throws IOException {
        ...
        BatchMetadata metadata = resolveMetadata(header, serviceType, clientRuc);
        BatchTotals totals = parseTotals(header, footer);
        ...
        return new ParsedBatch(
                ...
                header[3].trim(),
                hash,
                totals.headerRecords(),
                totals.headerAmount(),
                totals.footerRecords(),
                totals.footerAmount(),
                footer[0].trim(),
                details);
    }

    private BatchMetadata resolveMetadata(String[] header, String serviceType, String clientRuc) {
        ...
        return new BatchMetadata(resolvedRuc, resolvedServiceType);
    }

    private BatchTotals parseTotals(String[] header, String[] footer) {
        ...
        return new BatchTotals(headerRecords, headerAmount, footerRecords, footerAmount);
    }
    ...
    private record BatchMetadata(String clientRuc, String serviceType) {
    }

    private record BatchTotals(int headerRecords,
                               BigDecimal headerAmount,
                               int footerRecords,
                               BigDecimal footerAmount) {
    }
}
